<!--
 ...
 Description: Admin console layout shell (Vue port of legacy DashboardShell mode="admin")
              — P1 mechanical copy of layouts/dashboard.vue for the Vite SPA;
              template/styles unchanged; P2 wired the browser port of the
              admin shell (useAdminShellBrowser).
-->
...
//
// P2 note: the shell data (session user, permission capabilities, DreamUP
// events) resolves in the browser through `useAdminShellBrowser` — the SPA
// counterpart of the Nuxt `useAdminShell`. Its module-level promise cache
// deduplicates the shell reads between this layout and the admin pages, and
// keeps the session/permission material in memory only (never in Web Storage).
...
// Browser shell: session user, permission capabilities and DreamUP events
// (module-level in-memory dedup cache; failures fail closed).
const {
  currentUser,
  permissions,
  dreamUPEvents,
...
// Client-only fail-closed guard: a missing payload means the shell could not
// resolve the session (the Nitro gate normally redirects before this point).
...
// Full-document navigation keeps the legacy per-render server contract; in
// the SPA the P1 pages are placeholders, so external navigation is retained
// verbatim and P2 will revisit once real pages land.
