...
//
// Data reads follow the account-center contract: the session user and
// permission capabilities are resolved server-side through
// `server/queries/server-queries.ts` inside `useAsyncData` (server: true) and
// the serialized payload is reused on the client — no second request.
// The server-only modules are dynamically imported behind `import.meta.server`
// so they never leak into the browser bundle, and navigation between account
// pages keeps full-document semantics (external links) exactly like the
// legacy Next.js shell, so every render goes through the server query layer.
...
// SSR requests are redirected to /login by the server session helper; the
// payload key is shared with the account pages so it is never fetched twice.
const { currentUser, shellData, shellError } = await useAccountShell();
